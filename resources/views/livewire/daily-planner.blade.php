<div>
	<h2 class="mb-3 border-b-4 text-xl font-bold">{{ __(\Carbon\Carbon::parse($date)->format("l")) }}</h2>
			...
			@foreach ($meals as $meal)
				<tr class="mb-2 h-44 align-top">
					<td class="relative overflow-hidden border-b">
							...
							@foreach ($menu as $item)
								@if ($item->day == $date && $item->meal == $meal)
									<div class="mb-2 flex items-center text-sm">
										...
										<div wire:click="deleteMenuItem('{{ $meal }}',{{ $item->recipe->id }})" wire:confirm="Are you sure you want to delete this recipe?"
						...
						<button wire:click.live="openRecipeModal('{{ $meal }}')" class="absolute bottom-0 flex items-center text-sm text-gray-400"
							type="button">
