<div>
	...
	<ul>
		...
		<div class="flex items-center justify-between py-3 border-b-2 last:border-b-0">
			<li wire:sortable.handle>
				<div
					wire:click="$dispatch('openModal', { component: 'recipe-modal', arguments: {recipe: {{ $recipe }}} })">
					<h1 class="text-xl cursor-pointer">{{ $recipe->name }}</h1>
				</div>
			...
			<div class="text-sm inline-flex-1 min-w-max">
				<div wire:click="$dispatch('openModal', { component: 'recipe-modal', arguments: {recipe: {{ $recipe }}} })"
					class="flex items-center justify-end mb-2 cursor-pointer">
				...
				<div wire:click="delete({{ $recipe->id }})" wire:confirm="{{ __(" Are you sure you want to delete this
					recipe?") }}" class="flex justify-end text-xs text-red-500 cursor-pointer hover:text-red-700">
